...
# -*- coding: utf-8 -*-
"""
bubblesclothing.jp (BUBBLES) 卖空/补货/上新监控脚本
- Shopify 平台，通过全站 products.json 接口一次性获取全部商品
...
# ======================== 配置 ========================
BASE_URL = "https://bubblesclothing.jp"
COLLECTION_JSON_URL = "https://bubblesclothing.jp/products.json"
PAGE_SIZE = 250        # Shopify 单页最大值
MAX_PAGES = 3          # 最多翻 3 页（750 个商品，目前全站 144 个，1 页就够）
REQUEST_TIMEOUT = 20
...
STATE_FILE = os.environ.get("STATE_FILE", "/tmp/bubbles_monitor_state.json")
...
logger = logging.getLogger("bubbles-monitor")
...
def fetch_all_products():
    """
    用全站 products.json 接口一次性获取全部商品（含 SKU 卖空状态）
    目前 144 个商品，1 页 250 上限直接拿完；商品增多自动翻页
    """
    products = []
...
def _notify_priceup(events):
    if len(events) == 1:
        e = events[0]
        subject = f"📈 [bubbles] 回调: {e['product_name']} +{e['discount']}"
        title = f"[bubbles]回调:{e['product_name'][:15]} +{e['discount']}"
    else:
        subject = f"📈 [bubbles] {len(events)} 个SKU价格回调"
        title = f"[bubbles]{len(events)}个回调"
    rows = ""
    for e in events:
        img_html = f'<img src="{e["image"]}" style="max-width:120px;max-height:150px;border:1px solid #ddd;">' if e.get("image") else ""
    ...
    body = f'<html><body><h2 style="color:#27ae60;">📈 [bubbles] 价格回调</h2><p>{len(events)} 个SKU涨价:</p><table style="border-collapse:collapse;">{rows}</table></body></html>'
    desp = "### [bubbles] 价格回调\n\n"
    for e in events:
        desp += f"**{e['product_name']}**\n- SKU: {e['sku']}\n- {e.get('compare_txt', '')}→ **¥{e['new_price']:,}**（+{e['discount']}）\n- [查看商品]({e['url']})\n"
        if e.get("image"):
...
def _notify_soldout(events):
    if len(events) == 1:
        e = events[0]
        subject = f"🚨 [bubbles] 卖空: {e['product_name']} - {e['sku']}"
        title = f"[bubbles]卖空:{e['product_name'][:15]}"
    else:
        subject = f"🚨 [bubbles] {len(events)} 个SKU卖空"
        title = f"[bubbles]{len(events)}个SKU卖空"
    rows = ""
    for e in events:
        img_html = f'<img src="{e["image"]}" style="max-width:120px;max-height:150px;border:1px solid #ddd;">' if e.get("image") else ""
    ...
    body = f'<html><body><h2 style="color:#e74c3c;">🚨 [bubbles] 商品卖空告警</h2><p>{len(events)} 个SKU卖空:</p><table style="border-collapse:collapse;">{rows}</table></body></html>'
    desp = "### [bubbles] 卖空告警\n\n"
    for e in events:
        desp += f"**{e['product_name']}**\n- 货号: {e.get('number', '无')}\n- SKU: {e['sku']}\n- [查看商品]({e['url']})\n"
        if e.get("image"):
...
def _notify_restock(events):
    if len(events) == 1:
        e = events[0]
        subject = f"📦 [bubbles] 补货: {e['product_name']} - {e['sku']}"
        title = f"[bubbles]补货:{e['product_name'][:15]}"
    else:
        subject = f"📦 [bubbles] {len(events)} 个SKU补货"
        title = f"[bubbles]{len(events)}个SKU补货"
    rows = ""
    for e in events:
        img_html = f'<img src="{e["image"]}" style="max-width:120px;max-height:150px;border:1px solid #ddd;">' if e.get("image") else ""
    ...
    body = f'<html><body><h2 style="color:#27ae60;">📦 [bubbles] 补货通知</h2><p>{len(events)} 个SKU已补货上架:</p><table style="border-collapse:collapse;">{rows}</table></body></html>'
    desp = "### [bubbles] 补货通知\n\n"
    for e in events:
        desp += f"**{e['product_name']}**\n- 货号: {e.get('number', '无')}\n- SKU: {e['sku']}\n- [查看商品]({e['url']})\n"
        if e.get("image"):
...
def _notify_new(events):
    if len(events) == 1:
        e = events[0]
        subject = f"🆕 [bubbles] 上新: {e['product_name']}"
        title = f"[bubbles]上新:{e['product_name'][:15]}"
    else:
        subject = f"🆕 [bubbles] {len(events)} 个商品上新"
        title = f"[bubbles]{len(events)}个上新"
    rows = ""
    for e in events:
        img_html = f'<img src="{e["image"]}" style="max-width:120px;max-height:150px;border:1px solid #ddd;">' if e.get("image") else ""
    ...
    body = f'<html><body><h2 style="color:#e67e22;">🆕 [bubbles] 上新通知</h2><p>{len(events)} 个新商品上架:</p><table style="border-collapse:collapse;">{rows}</table></body></html>'
    desp = "### [bubbles] 上新通知\n\n"
    for e in events:
        desp += f"**{e['product_name']}**\n- 货号: {e.get('number', '无')}\n- SKU: {e['sku']}\n- [查看商品]({e['url']})\n"
        if e.get("image"):
...
def main():
    if "--test-notify" in sys.argv:
        notify_events([
            {"type": "SOLD_OUT", "product_name": "【测试】ハイネックボタンアンサンブル", "sku": "ブラックチェック / F", "number": "bs72300", "url": "https://bubblesclothing.jp/products/bs72300", "image": "https://cdn.shopify.com/s/files/1/0718/1978/8362/files/171_F_1_76059f89-25df-4140-b465-fd60cc528abd.jpg?v=1788932934", "time": datetime.now().isoformat()},
            {"type": "RESTOCK", "product_name": "【测试】ハイネックボタンアンサンブル", "sku": "ブラウンチェック / F", "number": "bs72300", "url": "https://bubblesclothing.jp/products/bs72300", "image": "https://cdn.shopify.com/s/files/1/0718/1978/8362/files/171_F_1_76059f89-25df-4140-b465-fd60cc528abd.jpg?v=1788932934", "time": datetime.now().isoformat()},
            {"type": "SALE", "product_name": "【测试】ハイネックボタンアンサンブル", "sku": "ブラックチェック / F", "number": "bs72300", "url": "https://bubblesclothing.jp/products/bs72300", "image": "https://cdn.shopify.com/s/files/1/0718/1978/8362/files/171_F_1_76059f89-25df-4140-b465-fd60cc528abd.jpg?v=1788932934", "new_price": 8800, "discount": "30%", "compare_txt": "~~¥12,600~~ →", "time": datetime.now().isoformat()},
            {"type": "NEW", "product_name": "【测试】ハイネックボタンアンサンブル", "sku": "ブラックチェック / F", "number": "bs72300", "url": "https://bubblesclothing.jp/products/bs72300", "image": "https://cdn.shopify.com/s/files/1/0718/1978/8362/files/171_F_1_76059f89-25df-4140-b465-fd60cc528abd.jpg?v=1788932934", "time": datetime.now().isoformat()},
        ])
        return
    ...
    if first_run:
        ...
        logger.info(f"bubbles 监控启动: 本轮全量扫描 {len(products)} 个商品 (已有状态 {products_with_skus} 个)")
    ...
    for p in products:
        ...
        # 货号 = handle（bubbles 的 handle 本身就是货号，如 bs72300）
        ...
        for sn, cp_info in cur_prices.items():
            old_p = prev_prices.get(sn)
            if not old_p:
                continue
            old_price = old_p.get("price", 0)
            ...
            if new_price > old_price:
                ...
                if rise < 2:   # 忽略 <2% 微调
                    continue
                sku_image = img_by_name.get(sn, "") or main_image
                events.append({"type": "PRICE_UP", "product_name": name, "sku": sn, "number": product_number, "url": url, "image": sku_image, "new_price": new_price, "discount": f"{rise}%", "compare_txt": f"¥{old_price:,} →", "time": datetime.now().isoformat()})
                logger.info(f"📈 回调: {name} - {sn} ¥{old_price:,}→¥{new_price:,}")
                continue
