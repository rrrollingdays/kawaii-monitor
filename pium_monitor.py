...
def _notify_priceup(events):
    if len(events) == 1:
        e = events[0]
        subject = f"📈 [pium] 回调: {e['product_name']} +{e['discount']}"
        title = f"[pium]回调:{e['product_name'][:15]} +{e['discount']}"
    else:
        subject = f"📈 [pium] {len(events)} 个SKU价格回调"
        title = f"[pium]{len(events)}个回调"
    rows = ""
    for e in events:
        img_html = f'<img src="{e["image"]}" style="max-width:120px;max-height:150px;border:1px solid #ddd;">' if e.get("image") else ""
    ...
    body = f'<html><body><h2 style="color:#27ae60;">📈 [pium] 价格回调</h2><p>{len(events)} 个SKU涨价:</p><table style="border-collapse:collapse;">{rows}</table></body></html>'
    desp = "### [pium] 价格回调\n\n"
    for e in events:
        desp += f"**{e['product_name']}**\n- SKU: {e['sku']}\n- {e.get('compare_txt', '')}→ **¥{e['new_price']:,}**（+{e['discount']}）\n- [查看商品]({e['url']})\n"
        if e.get("image"):
